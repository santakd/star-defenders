...
#   _)_/||
#
# star_defenders2.py - A production-grade implementation of the classic game using Pygame.
# Author: santakd
...
# Game Settings
PLAYER_SPEED = 7        # The player's ship moves 7 pixels per frame, which translates to 420 pixels per second at 60 FPS. This speed allows for responsive controls while still requiring skillful maneuvering to dodge alien attacks and position for shots.
...
def setup_logging():
    ...
    log_filename = os.path.join('logs', f"star_defenders2_{timestamp}.log")
...
class Game:
    def __init__(self):
        try:
            pygame.init()
            pygame.display.set_caption("Star Defenders 2 - Galactic Edition")
            self.screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))
            self.clock = pygame.time.Clock()
            
            # Use two different font sizes for hierarchy
            ...
            # --- STATS TRACKING ---
            self.shots_fired = 0           # Spacebar clicks
            self.total_bullets_fired = 0   # Total individual projectiles spawned
            self.bullets_hit = 0
            ...
            if event.type == pygame.KEYDOWN and not self.game_over and not self.victory:
                if event.key == pygame.K_SPACE:
                    ...
                    if bullets:
                        # 1 spacebar click
                        self.shots_fired += 1
                        # Track total individual projectiles (usually 2 per click for twin-blasters)
                        self.total_bullets_fired += len(bullets)
                        
                        logging.debug("Trigger Pressed count: %d", self.shots_fired)
    ...
    def update_logic(self):
        ...
        if self.game_over or self.victory:
            # If this is the first frame after the game ends, calculate and log stats
            if not self.stats_logged:
                self.duration_seconds = round((pygame.time.get_ticks() - self.start_time) / 1000.0, 1)
                
                # Count any bullets still in flight as missed
                self.bullets_missed += len(self.bullets)
                
                # Calculate Accuracy (prevent division by zero if player never shot)
                if self.total_bullets_fired > 0:
                    accuracy = (self.bullets_hit / self.total_bullets_fired) * 100
                else:
                    accuracy = 0.0
                ...
                logging.info(f"Trigger Pressed (Spacebar Clicks): {self.shots_fired}")
                logging.info(f"Total Shots: {self.total_bullets_fired}")
            ...
            # Wait for 10 seconds (20000ms), then close the game
            if pygame.time.get_ticks() - self.end_delay_started_time > 20000:
                self.running = False
                
            return # Freeze all other game logic while the 20-second timer runs

        # --- NORMAL GAME LOGIC ---
        self.all_sprites.update()
        
        # Track Missed Bullets: Check if any bullet went off screen
        ...
        # Returns a dict: {Alien: [list_of_bullets_that_hit_it]}
        hits = pygame.sprite.groupcollide(self.aliens, self.bullets, True, True)
        
        for alien, bullet_list in hits.items():
            self.score += 10
            # Accurately track how many actual bullets impacted the alien
    ...
    def draw(self):
        ...
        if self.game_over or self.victory:
            # 1. Draw main banner
            banner_text = "YOU WIN!" if self.victory else "GAME OVER"
            banner_color = GREEN if self.victory else RED
            banner_surf = self.font_large.render(banner_text, True, banner_color)
            
            # 2. Draw Stats Lines (Separated into 3 lines to fit cleanly on screen)
            stats_line1 = f"Game Duration: {self.duration_seconds}s  |  Accuracy Score: {self.accuracy_score}%"
            stats_line2 = f"Trigger Pressed: {self.shots_fired}  |  Total Shots: {self.total_bullets_fired} "
            stats_line3 = f"Total Hits: {self.bullets_hit}  |  Total Missed: {self.bullets_missed}"
            ...
            self.screen.blit(banner_surf, (center_x - banner_surf.get_width() // 2, center_y - 80))
            self.screen.blit(stats_surf1, (center_x - stats_surf1.get_width() // 2, center_y - 10))
            self.screen.blit(stats_surf2, (center_x - stats_surf2.get_width() // 2, center_y + 25))
            self.screen.blit(stats_surf3, (center_x - stats_surf3.get_width() // 2, center_y + 60))
